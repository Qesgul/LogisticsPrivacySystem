...
public class UnOverActivity extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.order_unover);
	...
	public class SubmitAsyncTask extends AsyncTask<String, Void, String> {
		...
		protected void onPostExecute(String result) {
			...
			if(res.isEmpty()){
				Toast.makeText(UnOverActivity.this, "服务器连接错误，请检查网络设置", Toast.LENGTH_SHORT).show();
			}else{
				text=res;
				String [] arr;
				arr=text.split("#");
				...
				orderadapter = new orderAdapter(UnOverActivity.this, R.layout.order_item, orderList);
				overlist=(ListView)findViewById(R.id.unover_list);
				overlist.setAdapter(orderadapter);
				overlist.setOnItemClickListener(new AdapterView.OnItemClickListener() {
					@Override
					public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
						Order order = orderList.get(position);
						Toast.makeText(UnOverActivity.this, "点击了"+order.getNum(), Toast.LENGTH_SHORT).show();
					}
				});
			}
	...
	private String doPost(String url){
		...
		try {
			...
			paramsList.add(new BasicNameValuePair("ORDER", "1"));
